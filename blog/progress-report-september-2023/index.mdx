...
The new system has a behavior model that is very closely related to how zombie behavior is processed, and this just creates a much faster and more reliable experience. 
...
- Simulate up to 10 of these patrols across the map with good performance (more to come)
...
We're setting a default lifetime of 1 week for all saved objects. This means that if you don't interact with a tent for 1 week, it will be deleted from the database.
...
These changes also apply to zombies, making them much more dangerous than they were previously, so be careful when engaging them in melee combat. We've really upped the shock damage values since the hit detection is so wonky right now.
...
Behind the scenes, we're continuing design for a comprehensive damage and repair system for vehicles, but we're not at a point where it makes sense to enable true damage yet, so we're keeping them indestructible for now. However, we have made some other quality-of-life improvements to how vehicles are handled in the world.
...
- Our modfiied time acceleration system will actually make nights shorter than days, so for those who don't like nighttime, we can make this pretty inconsequential (20 minute night vs 3 hour day etc) 
...
After this update, I'm going to be moving the project to a quarterly update cycle - this means that we'll be releasing updates every 3 months instead of every month. This will give us more time to work on larger features and content additions, and will also give us more time to test and polish the content we're releasing.
